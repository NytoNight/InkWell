...
class Take extends Talking
{
    ...
    private function insertANOrderDetails($orderId,OrderDetails $detail){
        try{
            ...
            $this->stat=$this->conn->prepare("UPDATE book set quantity=(Select quantity from book where book_id=?)-? where book_id=?");
            $this->stat->execute(array(
    ...
    private function undoOrder($orderId) {
        try{
            $this->stat=$this->conn->prepare("UPDATE book b SET quantity=(SELECT o.quantity FROM order_details o JOIN orders o3 on o.order_id = o3.order_id WHERE o3.order_id=? AND b.book_id=o.book_id )+(SELECT b.quantity FROM book c WHERE c.book_id=b.book_id) WHERE b.book_id in (SELECT x.book_id FROM book x JOIN order_details od on x.book_id = od.book_id JOIN orders o2 on od.order_id = o2.order_id WHERE o2.order_id=? );");
            $this->stat->execute([0=>$orderId,1=>$orderId]);
            $this->stat=$this->conn->prepare("DELETE FROM order_details WHERE order_id=?");
    ...
    public function takeBook(Book $book):bool {
        try{
            $this->stat=$this->conn->prepare("insert into book(book_id, title, author_id, publisher_id, genre_id, ISBN, sell_price, buy_price, image, description, quantity, actual_quantity) values (?,?,?,?,?,?,?,?,?,?,?,?);");
    ...
    public function updateBook(Book $book):bool{
        try{
            ...
            $this->stat=$this->conn->prepare("UPDATE book SET author_id=?,publisher_id=?,buy_price=?,sell_price=?,quantity=?,actual_quantity=?,image=? WHERE book_id=?;");
